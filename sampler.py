...
from images2vid import writeVideo
...
class Sampler():
  ...
  def generate_z(self):
    ...
    return z
  def generate(self, z=None, x_dim=1080, y_dim=1080, scale = 10.0):
    if z is None:
      z = self.generate_z()
    else:
  ...
  def save_anim_gif(self, z1, z2, filename, n_frame = 10, duration1 = 0.5, \
    ...
    for i in range(total_frames):
      theta = (1.0 * i) / total_frames * 2 * 3.14159
      print(theta)
      z = math.cos(theta) * z1 + math.sin(theta) * z2
      img = self.to_image(self.generate(z, x_dim, y_dim, scale))
      img.save("frames/frame_{0}.png".format(format(i, '03')))
      images.append(img)
      print "processing image ", i
    print "writing vid file..."
    writeVideo(filename, images)
